 ...
 * description Some variables are added but latter
 */
var locations = [{
    locationName: 'Gomti Nagar Greens',
    streetName: 'Gomti Nagar Railway Station Road, Vivek Khand, Near Neelkanth Sweets, Gomtinagar, Lucknow, Uttar Pradesh',
    zipCode: ' 226010',
    city: 'Lucknow',
...
mapObj.createMarkers = function() {
    ...
    self.displayAllMarkers = ko.computed(function() {
        ...
        for (var i = 0; i < locations.length; i++) {
            ...
            self.markerArray.push(self.marker); //storing markers in an array
...
/** Create a Google Map popup window */
...
mapObj.openWindow = function(marker, status) {
    var self = this;
    ...
    function openWin() {
        ...
        $('<div class="marker_title"><h4>' + marker.title + '</h4>' + '<h5>' + marker.description + '</h5></div>').appendTo('#windowTool'); //now add data
        //self.infowindow.setContent( '<div><h4>' + marker.title + '</h4>' + '<h5>' + marker.description + '</h5></div>');// unable to display image using knockoutJS
...
mapObj.bounceMarker = function(marker, status) {
    ...
    function bounce(marker) {
        ...
        //bounce the actual marker and unbounce it after few sec.
...
sidebarObj.searchLoc = function() {
    ...
    self.query.subscribe(search = function(value) {
        self.places.removeAll(); // remove other current places
...
photoObj.flickr = function(marker) {
    ...
    var string = 'https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=be17caa754fcee058bb87de7f2d281aa&jsoncallback=?';
    $.getJSON(string, {
            text: locationName,
            tagmode: 'any',
